...
static function img_EN(){
    ...
    // $gd_imgW = 140;
    // $gd_imgH = 50;
    $gd_imgW = self::$gd_imgW;
    $gd_imgH = self::$gd_imgH;
    $strLen = 4;
    $strLen = self::$strLen;
    ...
    $str = self::randWordArr_EN(4);
    var_dump($str);
    self::$str = $str;
     // 创建画布
    $img = imagecreatetruecolor(self::$gd_imgW,self::$gd_imgH);
    $imgW = imagesx($img);
    $imgH = imagesy($img);
        // 背景颜色
    ...
    for ($i=0; $i < $strLen ; $i++){
        ...
        echo PUBLIC_PATH.'captcha0.ttf';
        imagettftext($img,$fontSize,mt_rand(-30,30),$imgW/$strLen*$i+($imgW/$strLen/2)-($fontSize/2),mt_rand($fontSize,$imgH),$fontColor,'captcha0.ttf',$str[$i]);
    }
        // 添加噪点
